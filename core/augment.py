...
class BakeAugment(nn.Module):
    ...
    def apply_oklabp_curve(self, input_t, target_t):
        ...
        # 1. 명도(L) 대비 왜곡
        ctrl_x_L, ctrl_y_L = self._make_random_curve(B, 399, 0.25, device, dtype)
        delta_L = self._apply_curve(L_tgt, ctrl_x_L, ctrl_y_L) - L_tgt
        L_out = L_in + delta_L

        # 2. a채널(Green-Red) 균형 왜곡
        ctrl_x_a, ctrl_y_a = self._make_random_curve(B, 399, 0.25, device, dtype)
        delta_a = self._apply_curve(a_tgt, ctrl_x_a, ctrl_y_a) - a_tgt
        a_out = a_in + delta_a

        # 3. b채널(Blue-Yellow) 균형 왜곡
        ctrl_x_b, ctrl_y_b = self._make_random_curve(B, 399, 0.25, device, dtype)
