...
class BakeAugment(nn.Module):
    ...
    def forward(self, x):
        ...
        degradations = [
            lambda inp: self.apply_oklabp_curve(inp, target, strength=0.3),
            lambda inp: self.apply_hsl(inp, target, strength=0.3),
            lambda inp: self.apply_color_wheels(inp, target, strength=0.3),
        ]
