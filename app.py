...
def process_single(image_path: str | None, bit_depth: str):
    ...
    tmp = tempfile.NamedTemporaryFile(suffix="_bake.png", delete=False)
    tmp_path = tmp.name
    tmp.close()
    save_tensor_to_8bit_png(output_tensor, tmp_path)
...
def process_batch(
    ...
    for file_path in progress.tqdm(files, desc="Processing"):
        ...
        try:
            ...
            save_tensor_to_8bit_png(result, os.path.join(tmp_dir, f"{name}_bake.png"))
...
with gr.Blocks(title="Bake") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Single Image"):
            ...
            single_download = gr.File(label="PNG Download", interactive=False)
